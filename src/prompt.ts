...
// In binary: falls back to embedded constant
...
// Embedded fallback — identical to prompts/synthesize.md
// Ensures binary distribution never silently fails because the file is missing.
// Keep in sync: if you edit prompts/synthesize.md, update this constant too.
const EMBEDDED_SYNTHESIS_PROMPT = `You are a senior developer writing a concise session handoff. Your audience is the next developer (or AI) who opens this project cold tomorrow. Write what they need to know — not what changed in the diff.
...
export async function loadPromptTemplate(): Promise<string> {
  ...
  try {
    ...
    // Filesystem read failed (binary distribution) — use embedded fallback
    cachedPrompt = EMBEDDED_SYNTHESIS_PROMPT;
    return cachedPrompt;
  }
}
