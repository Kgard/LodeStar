...
- [-] **lodestar_synthesize — session synthesis via LLM** — 90% — Google and Azure providers added as uncommitted files; @google/generative-ai package added. Provider index registration and end-to-end routing still need verification after commit.
...
- [x] **Cursor AI integration** — 100%
  ...
  - ✓ Trigger phrases
- [-] **lodestar diff — drift detection** — 75% — Updated via commit: chore: update session context via lodestar end
  - ○ Drift detection between current codebase and last synthesis
- [-] **Multi-provider expansion (Google, Azure)** — 65% — src/providers/google.ts and src/providers/azure.ts exist as untracked files. @google/generative-ai added to package.json (uncommitted). Provider index registration and init wizard wiring still pending.
  - ○ Google Gemini provider implementation
  - ○ Azure OpenAI provider implementation
...
- Commit src/providers/google.ts, src/providers/azure.ts, and package.json — then verify provider index registration in src/providers/index.ts routes to them correctly
- Run end-to-end synthesis with Google and Azure providers to confirm LLMProvider interface compliance
- Verify src/init.ts wizard exposes Google and Azure as selectable options and stores correct provider keys in ~/.lodestar.config.json
- Review all other uncommitted files (src/config.ts, src/index.ts, src/synthesize.ts, src/schema.ts, src/summary.ts, src/review.ts, src/merge.ts, prompts/synthesize.md) — determine if they are ready to commit
