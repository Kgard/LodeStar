...
- [-] **lodestar synthesize — two-diff session synthesis** — 90% — Bug fixed this session: .lodestar.md is now filtered out of the uncommitted diff before checking hasUncommitted, preventing synthesize from no-oping when only the output file has changed.
- [-] **lodestar load — structured context loader** — 75% — Remaining gaps not yet identified. Needs audit of src/load.ts.
- [-] **lodestar review — browser-based session reader** — 95% — Possible escapeHtml/markdownToHtml ordering bug unverified. Run lodestar review and inspect a markdown-containing panel.
- [ ] **lodestar diff — drift detection** — 10% — Phase 1b stub only. Do not implement until gates in CLAUDE.md are met.
...
- [BLOCKING] Does markdownToHtml() in template.ts have a bug where markdown symbols are HTML-escaped by escapeHtml() before the markdown regexes run, causing regexes to never match? Run lodestar review and inspect a panel with markdown content to confirm or rule out.

## Next Session

- Commit the two uncommitted files: .lodestar.md and src/synthesize.ts.
- Verify escapeHtml()/markdownToHtml() ordering bug — run lodestar review and inspect a markdown-containing panel to confirm or rule out the rendering issue.
- Audit src/load.ts to identify what the remaining 25% gaps are before starting new features.
- Check CLAUDE.md for Phase 1b implementation gates before touching lodestar diff.
