...
- [-] **Terminal summary — distilled session briefing** — 45% — Updated via commit: feat: terminal summary — 5-line session briefing for Session
...
**Rationale:** SessionStart hook must be fast and frictionless (5-10 seconds max); full context reconstruction belongs in a manual browser tool. Terminal summary prints distilled nextSession bullets, most recent rejection, and blocking questions to stderr; lodestar review provides full .lodestar.md visualization. Prevents information overload at session start while keeping depth accessible.
...
**Rationale:** Reduces visual clutter on page load while keeping diagrams available. Progressive disclosure matches lightweight page design.
