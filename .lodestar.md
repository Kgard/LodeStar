...
- [-] **lodestar_synthesize — session synthesis via LLM** — 95% — Google and Azure providers exist as untracked files; provider index registration pending.
...
- [x] **Cursor AI integration** — 100%
  ...
  - ✓ Trigger phrases
- [-] **lodestar diff — drift detection** — 80% — Updated via commit: chore: update session context via lodestar end
...
- Complete src/providers/google.ts — implement LLMProvider interface using @google/generative-ai (package already added to package.json)
- Complete src/providers/azure.ts — implement LLMProvider interface for Azure OpenAI
- Register Google and Azure providers in src/providers/index.ts so they are routable from synthesis
- Wire Google and Azure into src/init.ts provider selection wizard so users can choose them during setup
- Commit all multi-provider files once end-to-end synthesis routing is verified for at least one new provider
