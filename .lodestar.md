...
- [-] **Session Capture** — 86% — Reads what changed in the codebase (uncommitted and committed diffs), captures high-level decisions from brief files separately, sends to an LLM, and produces a structured .lodestar.md with decisions, patterns, features, and open questions — enabling any AI tool to resume with full context.
...
- [-] **Plumbing & Integrations** — 70% — The infrastructure that makes everything else work — git hooks that auto-commit context, CI that builds cross-platform binaries, and integrations with GitHub, Beehiiv, and distribution channels. Handles the machinery so features can be reliable.
...
- Implement lodestar start — explicit session-start command for Cursor/Windsurf users without git hooks
- Build full bootstrap mode to read existing codebase and generate real PRD from project structure
- Add project preferences file (.lodestar.preferences) so users can customize synthesis without editing code
- Complete EULA/license enforcement module for binary distribution
- Validate multi-session near-duplicate dedup to ensure older decisions aren't resurfaced when titles change slightly
