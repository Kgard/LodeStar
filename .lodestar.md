...
- [-] **Session Capture** — 87% — Reads what changed in the codebase (uncommitted and committed since last synthesis), extracts high-level decisions from brief files, and produces a structured .lodestar.md that persists your reasoning across AI coding sessions.
...
- [-] **Session Continuity** — 78% — Automatically saves context at session start and end via git hooks, loads the last session summary to terminal in 10 seconds, and registers MCP tools for Claude Code to load full context on demand.
...
- Review .cursorrules modifications in git status to confirm they reflect current synthesis context paths and provider configuration
- Verify EULA.md is complete and accurately reflects all telemetry, auto-update, and license restrictions from existing decisions
- Test end-to-end init → synthesis → review flow to confirm Loops email capture and .cursorrules generation work correctly
- Check loop-emails/ directory structure to ensure email templates align with Loops integration
