...
- [x] **lodestar_synthesize — session synthesis via LLM** — 100% — Updated via commit: chore: update session context via lodestar end
...
- [x] **Terminal summary — distilled session briefing** — 100% — Updated via commit: chore: update session context via lodestar end
...
- [x] **Cursor AI integration** — 100%
  ...
  - ✓ Trigger phrases
- [-] **lodestar diff — drift detection** — 85% — Drift detection between current codebase and last synthesis
  - ○ Drift detection between current codebase and last synthesis
  - ○ Drift detection implementation
- [-] **Multi-provider expansion (Google, Azure)** — 75% — src/providers/google.ts and src/providers/azure.ts created as untracked files. @google/generative-ai package added. Provider index registration and init wizard wiring pending.
  - ○ Google Gemini provider implementation
  - ○ Azure OpenAI provider implementation
...
- Complete Google Gemini and Azure OpenAI provider integration: register in src/providers/index.ts, wire into init.ts provider selection, and test end-to-end synthesis routing
- Verify src/merge.ts correctly unions decisions and preserves diagrams across synthesis cycles — add test coverage for merge semantics
- Implement .lodestar.synthesizing lockfile cleanup on graceful and error exits to prevent stale sentinel files
- Add package.json @google/generative-ai dependency to committed changes and verify Azure OpenAI SDK is also added
