...
> Session Duration: single handoff synthesis
...
- [-] **Session Capture** — 87% — Reads what changed in the codebase (uncommitted and committed diffs), captures high-level decisions from brief files separately, sends to an LLM, and produces a structured .lodestar.md with decisions, patterns, features, and open questions — enabling any AI tool to resume with full context.
...
- [-] **Session Continuity** — 78% — Automatically saves context at session start and end via git hooks, loads previous context into memory for the LLM to reconcile, and displays a 5-line summary in the terminal so the developer knows what was recovered — eliminating context-switching friction.
...
- [x] **Session Review** — 100% — Opens a local browser page with three disclosure levels that lets you understand the last session in 5 seconds (Level 1: project + feature status) or reconstruct full context in 2 minutes (Level 3: all decisions, diagrams, and rejected approaches) — the screenshot people share.
...
- [x] **Developer Onboarding** — 100% — One command (lodestar init) picks your AI provider, validates your API key, wires up git hooks, and configures your coding tool — zero manual config editing needed to start saving session context.
...
- [-] **Plumbing & Integrations** — 85% — The infrastructure that makes everything else work — git hooks that auto-commit context, CI that builds cross-platform binaries, and integrations with GitHub, Beehiiv, and distribution channels. Handles the machinery so features can be reliable.
...
- [ ] **Pro Tier** — 0% — Where revenue lives — hosted synthesis (no BYOK), mid-session checkpoints, session diffs, 30-day history, and adaptive learning. $9.99/month via Kylex Pro.
...
- Implement lodestar start — explicit session-start command for Cursor/Windsurf users without relying on git hooks
- Build full bootstrap mode to read existing codebase and generate real PRD from project structure instead of [UNKNOWN] placeholders
- Add project preferences file (.lodestar.preferences) so users can customize synthesis rules without editing code
- Complete EULA/license enforcement module to gate binary distribution and set up license telemetry
- Validate multi-session near-duplicate dedup across 3+ synthesis runs to ensure older decisions aren't resurfaced when decision titles change slightly
