...
- [-] **Session Capture** — 87% — Reads both uncommitted and committed code changes since last synthesis, separates brief changes as first-class decisions, and produces a structured .lodestar.md via LLM synthesis with merge-layer reconciliation.
...
- [-] **Session Continuity** — 78% — Automatically saves context between AI coding sessions via git hooks, provides a 5-line terminal briefing at session start, and exposes MCP tools for Claude Code and Cursor integration.
...
- [-] **Plumbing & Integrations** — 86% — Infrastructure that makes everything else work—git hooks that auto-commit context, CI that builds cross-platform binaries, self-updating binary from GitHub Releases, email capture via Loops, and anonymous telemetry tracking. Handles the machinery so features can be reliable.
...
- Implement EULA enforcement — generate EULA.md with binary terms, require acceptance before first synthesis, store acceptance status in config
- Test Loops email integration end-to-end: validate API key in init, test welcome email delivery on signup, confirm list creation in Loops dashboard
- Verify CI/CD release workflow stability — test concurrent independent jobs, confirm no race conditions in binary uploads, validate notarization timeout handles real-world delays
