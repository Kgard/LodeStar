...
- [-] **lodestar synthesize — two-diff session synthesis** — 82% — Two-diff architecture implemented. synthesize.ts and prompts/synthesize.md modified this session. Type-check passes per verified-resolved evidence. markdownToHtml() escapeHtml() ordering bug still unverified.
- [-] **lodestar load — structured context loader** — 70% — Updated but not fully verified end-to-end.
- [-] **lodestar review — browser-based session reader** — 85% — Core committed. extractHtmlBody(), markdownToHtml(), futurePhases rendering, PRD panel, and raw GitHub URL display exist but have not been verified end-to-end — no commits confirmed this session.
- [ ] **lodestar diff — drift detection** — 5% — Phase 1b stub only per CLAUDE.md. Do not implement yet.
...
**Rationale:** Because .lodestar.md is committed on every synthesis, any machine that can git pull has access to the latest context. Web-based AI tools can use the raw GitHub URL pattern to fetch context directly.
**Files:** CLAUDE.md
...
- [BLOCKING] Does markdownToHtml() in template.ts have a bug where markdown symbols are HTML-escaped by escapeHtml() BEFORE the markdown regexes run, causing the regexes to never match? Would silently produce unrendered markdown in PRD panels.

## Next Session

- Audit markdownToHtml() in template.ts for the escapeHtml()-before-regex ordering bug — highest-probability silent defect, inspect call order directly in the source.
- Run lodestar synthesize then lodestar review end-to-end: verify PRD panel renders real markdown, raw GitHub URL appears, and futurePhases renders as a distinct section.
- Commit all dirty files (.lodestar.md, prompts/synthesize.md, src/synthesize.ts) as a single atomic commit once end-to-end test passes — they are interdependent.
- Investigate src/verify-questions.ts (untracked file) — determine if it belongs in the repo or should be gitignored.
