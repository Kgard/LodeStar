...
- [x] **Cursor AI integration** — 100%
  ...
  - ✓ trigger phrases
- [-] **lodestar diff — drift detection** — 90% — Drift detection between current codebase and last synthesis
...
- Verify release CI completes successfully after the macos-latest runner fix (commit 852d76e) — check GitHub Actions for all three build targets (macos-arm64, macos-x64, linux-x64)
- Complete lodestar diff drift detection feature (currently 90%) — the last remaining in-progress Phase 1 item
- Begin full bootstrap mode: enable LLM to read source files and generate a real PRD from existing code (src/bootstrap.ts)
