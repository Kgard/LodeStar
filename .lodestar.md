...
- **Loops (loops.so)** [api] — Transactional + marketing email for welcome emails on signup and newsletter distribution
- **GitHub Releases API** [api] — Version check and self-updating binary distribution

## Project Brief Status

- [-] **Session Capture** — 87% — Reads what changed in the codebase, sends it to an LLM, and produces a structured .lodestar.md — the core synthesis engine that makes everything else possible.
...
- [-] **Session Continuity** — 78% — Ensures every AI session starts with full project context — auto-loads .lodestar.md at session start, fires synthesis at session end, and provides MCP tools so the AI can query context on demand.
...
- [x] **Session Review** — 100% — Opens a local browser page with three disclosure levels—5-second overview, feature and decision status, full diagrams and open questions—so you understand the last session at any depth in 2 minutes.
...
- [x] **Developer Onboarding** — 100% — One command (lodestar init) picks your AI provider, validates your API key, wires up git hooks, configures your coding tool, generates .cursorrules, captures email for product updates, and integrates Loops API—zero manual config editing needed to start saving session context.
  ...
  - ✓ Email capture integration (Loops API)
...
- [-] **Plumbing & Integrations** — 87% — Infrastructure that makes everything else work—git hooks that auto-commit context, CI that builds cross-platform binaries, self-updating binary from GitHub Releases, email capture via Loops, and anonymous telemetry tracking.
  ...
  - ✓ Email capture integration (Loops API)
  - ✓ Node.js 24 opt-in for GitHub Actions runners
- [ ] **Pro Tier** — 0% — Revenue model via capability gating—hosted synthesis (no BYOK), mid-session checkpoints, session diffs, 30-day history, and adaptive learning. Pro gates features requiring server-side infrastructure.
...
### Email provider changed from Beehiiv to Loops (loops.so)
**Status:** active
**Group:** Product & Business
**Session:** 2025-04-23
**Rationale:** Loops provides both transactional and marketing email in one platform with API available for triggered welcome emails on signup. Simpler unified platform compared to newsletter-native Beehiiv. Beehiiv, Mailchimp, and ConvertKit explicitly rejected.
**Files:** CLAUDE.md

### Split release CI into independent jobs with manual dispatch to prevent concurrent workflow cancellations
**Status:** active
**Group:** Core Architecture
**Session:** 2025-04-23
**Rationale:** Parallel release jobs were cancelling each other mid-build. Independent jobs with manual dispatch allow intentional orchestration without race conditions. Reduces CI fragility and improves release reliability.
**Files:** .github/workflows/release.yml

### Increase macOS notarization timeout to 30 minutes
**Status:** active
**Group:** Core Architecture
**Session:** 2025-04-23
**Rationale:** macOS notarization can take longer than the default timeout under heavy load. Extended timeout prevents timeout errors during release build while maintaining the security requirement of notarization.
**Files:** .github/workflows/release.yml

### Add email capture to init wizard and review reader
**Status:** active
**Group:** Onboarding
**Session:** 2026-04-05
**Rationale:** Integrates Loops API calls into the onboarding flow and review UI to build newsletter subscriber list. Captures user interest at two key moments: initial setup and context review.
**Files:** src/init.ts, src/review.ts

### Opt into Node.js 24 for GitHub Actions runners
**Status:** active
**Group:** Core Architecture
**Session:** 2026-04-06
**Rationale:** Commit c7ae081 explicitly opts into Node.js 24 for GitHub Actions runners, likely to address deprecation warnings or align with the current LTS. Keeps CI runtime consistent with current Node.js ecosystem.
**Files:** .github/workflows/release.yml
...
- Only a Node.js 24 opt-in CI chore landed in this session. No product changes — continue with Phase 1b priorities.
- Implement lodestar start command for Cursor/Windsurf users who lack hook support.
- Complete full bootstrap mode (src/bootstrap.ts) — LLM reads source files to generate a real PRD instead of [UNKNOWN] placeholders.
- Address EULA outstanding decision — binary ships legally unprotected without it.
- Validate multi-session dedup in merge layer across several real synthesis cycles.
