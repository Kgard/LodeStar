...
- [x] **lodestar init — first-run CLI wizard** — 100% — Provider selection, key validation, config creation, retry on invalid directory, and skip-prompt-if-valid-config all implemented.
- [x] **lodestar synthesize — two-diff session synthesis** — 100% — Priority-based diff truncation via src/diff-priority.ts. Dual-model routing: checkpoint (fast, mid-session) and full (comprehensive, end-of-session). Now separates brief file diffs from code diffs.
...
- [-] **lodestar diff — drift detection** — 30% — Phase 1b feature. Core drift detection logic not yet implemented.
...
**Rationale:** Checkpoint saves (lodestar save --quick) should be fast and cost-effective; full syntheses at session end can use more powerful models for comprehensive analysis. Model selection is based on synthesis mode, not user config.
...
**Rationale:** Without this filter, synthesize would see its own output file as an uncommitted change, causing false positives on hasUncommitted.
...
- Test brief diff extraction end-to-end: verify CLAUDE.md/PRD.md changes appear in separate briefDiff channel and are passed correctly to the prompt
- Verify prompt integration: confirm the new brief diff section is rendered correctly in the synthesis input template and model sees both code and brief changes
