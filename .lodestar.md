...
- [x] **Session Capture** — 92% — Reads what changed in the codebase, sends it to an LLM, and produces a structured .lodestar.md that captures decisions, rationale, and rejected approaches — not just diffs
...
- [-] **Session Continuity** — 85% — Ensures every new session starts warm — loads previous context, prints a quick briefing, and fires hooks so the developer (or AI) never opens the project cold
...
- [-] **Plumbing & Integrations** — 80% — The infrastructure that makes everything else work — git hooks that auto-commit context, CI that builds cross-platform binaries, and integrations with GitHub, Beehiiv, and distribution channels
