...
- [x] **lodestar synthesize — two-diff session synthesis** — 100% — Now uses priority-based diff truncation via src/diff-priority.ts — splits diffs by file, trims low-priority files first, and reports excluded file names in warnings.
- [-] **lodestar load — structured context loader** — 95% — Updated via previous session.
- [x] **lodestar review — browser-based session reader** — 100%
- [ ] **lodestar diff — drift detection** — 15%
...
- [non-blocking] What priority ordering does src/diff-priority.ts use to rank files? The file is untracked and not visible in the diff — the ranking logic is unknown.

## Next Session

- Review src/diff-priority.ts (untracked, not yet committed) — verify splitDiffByFile and truncateByPriority logic and add tests
- Confirm the character/4 token estimate is close enough in practice; replace with provider.countTokens() if accuracy matters for edge cases
- Begin lodestar diff — drift detection (currently 15% complete, next planned phase)
