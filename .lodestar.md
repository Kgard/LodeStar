...
- [-] **lodestar synthesize — two-diff session synthesis** — 90% — Bug fixed last session: .lodestar.md is filtered out of uncommitted diff before hasUncommitted check. No changes this session.
...
- Remove the stray `// test` comment added to src/cli.ts and commit .lodestar.md and src/synthesize.ts.
- Verify escapeHtml()/markdownToHtml() ordering bug — run `lodestar review` and inspect a panel with markdown content to confirm or rule out.
