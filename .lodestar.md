...
- [x] **lodestar init — first-run CLI wizard** — 100% — Provider selection, key validation, config creation, retry on invalid directory, skip-prompt-if-valid-config all implemented. Now also writes .cursorrules when Cursor is selected.
...
- [-] **lodestar diff — drift detection** — 35% — Phase 1b. Tool description updated to clarify intent but core logic not yet implemented.
- [-] **Cursor AI integration — .cursorrules + MCP trigger phrases** — 70% — init.ts writes .cursorrules when Cursor selected. MCP tool descriptions updated with trigger phrases. cursorrules.template exists in repo root (untracked). Init flow wires .cursorrules only at init time — not retroactively for existing projects.
...
- [non-blocking] cursorrules.template exists as an untracked file in the repo root — should it be committed as the canonical template, or is it a scratch file that can be deleted now that the template is inlined in src/init.ts?

## Next Session

- Decide fate of cursorrules.template (untracked) — commit it or delete it
- Test Cursor integration end-to-end: run lodestar init, select Cursor, verify .cursorrules written correctly, verify trigger phrases invoke lodestar_load/lodestar_synthesize
- Add a lodestar_cursorrules command or --cursor flag to inject .cursorrules into existing projects that already ran init before this feature existed
- Begin Phase 1b: implement lodestar_diff core logic in src/diff.ts
