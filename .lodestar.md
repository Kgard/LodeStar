...
- [-] **Session Capture** — 86% — Reads what changed in the codebase, sends it to an LLM, and produces a structured .lodestar.md that captures decisions, rationale, and rejected approaches — not just diffs
...
- [-] **Session Continuity** — 78% — Ensures every new session starts warm — loads previous context, prints a quick briefing, and fires hooks so the developer (or AI) never opens the project cold
...
- [x] **Session Review** — 100% — Opens a local browser page that lets you understand the last session in 5 seconds (Level 1) or reconstruct full context in 2 minutes (Level 3) — the screenshot people share
...
- [x] **Developer Onboarding** — 100% — One command (`lodestar init`) picks your AI provider, validates your API key, wires up hooks, and configures your coding tool — zero manual config editing
...
- [x] **Provider System** — 100% — Synthesis works with any LLM — Anthropic, OpenAI, Ollama, Google, or Azure — same prompt, same output, user picks during init
...
- [-] **Plumbing & Integrations** — 70% — The infrastructure that makes everything else work — git hooks that auto-commit context, CI that builds cross-platform binaries, and integrations with GitHub, Beehiiv, and distribution channels
...
- Review uncommitted changes to src/cli.ts, src/config.ts, src/init.ts — verify they relate to lodestar start command or bootstrap mode implementation for Phase 1b
- Test macOS runner change (macos-latest in .github/workflows/release.yml) — confirm builds succeed on both arm64 and x64 targets
- Implement lodestar start command for explicit session initiation (Phase 1b priority)
