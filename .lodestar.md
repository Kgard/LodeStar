...
- [-] **lodestar load — structured context loader** — 65% — Updated via commit: feat: git hooks for automatic context updates + prompt improvements.
...
- [BLOCKING] Does src/synthesize.ts correctly thread committedDiff and commitLog from GitSnapshot into the {{committed_diff}} and {{commit_log}} prompt template variables? Critical integration point for the two-diff architecture — unverified end-to-end.
...
- Run npm run build — the five uncommitted files (template.ts, schema.ts, review.ts, synthesize.ts, prompts/synthesize.md) must type-check together before anything is verifiable.
