...
- [-] **Session Capture** — 87% — Reads what changed in the codebase (uncommitted + committed diffs), separates brief changes (CLAUDE.md, PRD.md) as first-class decisions, truncates by file priority to stay within budget, and sends to an LLM to produce a structured .lodestar.md with decisions, patterns, diagrams, and rejected approaches.
...
- [-] **Session Continuity** — 78% — Auto-loads the last .lodestar.md context into Claude Code, Cursor, or Windsurf at session start via MCP tools, shows a 5-line terminal briefing for momentum, and auto-commits/pushes before and after synthesis via git hooks.
...
- [x] **Session Review** — 100% — Opens a local browser page with three disclosure levels so you understand the last session in 5 seconds (Level 1: project + feature status) or reconstruct full context in 2 minutes (Level 3: all decisions, diagrams, and rejected approaches).
...
- [-] **Plumbing & Integrations** — 85% — The infrastructure that makes everything else work — git hooks that auto-commit context, CI that builds cross-platform binaries, and integrations with GitHub for distribution. Handles the machinery so features can be reliable.
...
- Review .cursorrules changes — verify Cursor receives updated context paths and provider hints correctly on next init
- Begin Phase 1b: implement lodestar diff command for drift detection between current codebase and last synthesis
- Plan EULA content and license enforcement mechanism for binary distribution phase
